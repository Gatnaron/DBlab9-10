...
namespace ProgForDB
{
    public class ReportService
    {
        ...
        public void DisplayDefaultReport(DataGridView dataGrid)
        {
            ...
            adapter = new SqlDataAdapter("select cl.client_id as id, cl.first_name as Имя, cl.last_name as Фамилия, cl.phone as Телефон, " +
                "COUNT(ord.order_id) as Количество_товаров, SUM(prod.price) as Всего_потрачено" +
            " from client cl" +
            " join orders ord ON ord.client_id = cl.client_id" +
            " join product prod ON prod.product_id = ord.product_id" +
            " group by cl.client_id, cl.first_name, cl.last_name, cl.phone", connection);
            adapter.Fill(dataTable);
            dataGrid.DataSource = dataTable;
            connection.Close();
        }

        public void DisplayBCReport(DataGridView dataGrid, string year)
        {
